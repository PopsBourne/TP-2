...
public class Vehiculo extends ObjetoSimulacion {
	...
	public Vehiculo(String id, int velocidadMaxima, List<CruceGenerico<?>> iti) throws NoExiste {
		super(id);
	...
	public void moverASiguienteCarretera() {
		...
		if(this.carretera != null) {
			...
			CruceGenerico<?> c1 = carretera.getCruceDestino();
			...
			if (cont == itinerario.size()-1) {
				...
				CruceGenerico<?> c2 = itinerario.get(cont+1);
			...
			CruceGenerico<?> c1 = itinerario.get(0);
			CruceGenerico<?> c2 = itinerario.get(1);
